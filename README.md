...
ApplicationSetting.columnSetting.column_names.select { |c| c.end_with?("_encrypted") }.each do |enc_col|
...
puts "JWT Key: #{s.ci_jwt_signing_key.present? ? "✅" : "❌"}"
puts "Job Token Key: #{s.ci_job_token_signing_key.present? ? "✅" : "❌"}"
'
Если NULL или CipherError - пересоздайте (см. раздел Sign-up Settings, Вариант 2).
Q: Можно ли использовать backup из GitLab 16.x для восстановления в 17.x?
A: Да, но сначала восстановите в той же версии, затем обновляйтесь поэтапно:
16.11 → 17.0 → 17.6 (latest)
Q: Как часто нужно делать backup gitlab-secrets.json?
A: При каждом backup БД! Также сохраняйте его при:
Изменении конфигурации GitLab
Обновлении версии
Миграции на новый сервер
📚 Дополнительные ресурсы
GitLab Backup Documentation
...
🐛 Создайте issue с выводом команды диагностики
📄 Лицензия
MIT License
